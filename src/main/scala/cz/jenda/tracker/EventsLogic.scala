...
class EventsLogic(dao: Dao, subscriptions: Subscriptions) {
          ...
          .flatMap {
            case Some(rel) =>
              ...
              dao.updateVisitedWaypoints(trackId, visitedWaypoints) >>
                dao.save(coords) >>
                subscriptions.newCoordinates(coords) >>
                logger.info(s"Received new coordinates from tracker ID $trackerId for track ID ${coords.trackId}")
